...
class Bank extends CI_Controller {
	...
	function _view(){ 
		...
		$this->site->write('title',getLanguage('ngan-hang'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['bank_name'];
		$this->base_model->addAcction(getLanguage('ngan-hang'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['bank_name'];
		$this->base_model->addAcction(getLanguage('ngan-hang'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
