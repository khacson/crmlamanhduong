...
<div class="portlet box blue mtop0">
	<div class="portlet-title">
		<div class="caption">
			 <div class="brc mtop3"><i class="fa fa-bars"></i> <?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('ngan-hang');?></div>		
		</div>
		<div class="tools">
					...
					<div id="gridView">
						<table id="tbbody" width="100%" cellspacing="0" border="1">
							...
							<tr class="row-search">
								...
								<td></td>
								<td>
									<input type="text" name="bank_code" id="bank_code" class="searchs form-control " />
								</td>
								<td>
									<input type="text" name="bank_name" id="bank_name" class="searchs form-control " />
								</td>
								<td>
...
<script>
	...
	function searchFunction(){
		$("#bank_code,#bank_name,#description").keyup(function() {
			searchList();	
		});
	}
	...
	function save(){
		...
		if(obj.bank_owner == ''){
			warning('<?=getLanguage('chu-tai-khoan-khong-duoc-trong');?>'); return false;	
		}
		if(obj.bank_code == ''){
			warning('<?=getLanguage('tai-khoan-khong-duoc-trong');?>'); return false;	
		}
		if(obj.bank_name == ''){
			warning('<?=getLanguage('ten-ngan-hang-khong-duoc-trong');?>'); return false;	
		}
		var token = $('#token').val();
		$('.loading').show();
		$.ajax({
    ...
    function funcList(obj){
		$('.edit').each(function(e){
			$(this).click(function(){ 
				 var bank_code = $('.bank_code').eq(e).html().trim();
