...
<div class="portlet box blue mtop0">
	<div class="portlet-title">
		<div class="caption caption2">
			 <div class="brc mtop3"><i class="fa fa-bars"></i> <?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('tram-dang-kiem');?></div>			
		</div>
		<div class="tools">
					...
					<div id="tHeader">    	
						<table id="tbheader" width="100%" cellspacing="0" border="1" >
							...
							<tr>							
								...
								<th id="ord_c.registrationstation_code"><?=getLanguage('ma-tram');?></th>
								<th id="ord_c.registrationstation_name"><?=getLanguage('ten-tram');?></th>
								<th id="ord_c.registrationstation_address"><?=getLanguage('dia-chi');?></th>
								<th id="ord_c.registrationstation_phone"><?=getLanguage('dien-thoai');?></th>
								<th></th>
								<th></th>
							</tr>
						</table>
					...
					<div id="gridView">
						<table id="tbbody" width="100%" cellspacing="0" border="1">
							...
							<tr class="row-search">
								...
								<td></td>
								<td>
									<input type="text" name="registrationstation_code" id="registrationstation_code" class="searchs form-control " />
								</td>
								<td>
									<input type="text" name="registrationstation_name" id="registrationstation_name" class="searchs form-control " />
								</td>
								<td>
									<input type="text" name="registrationstation_address" id="registrationstation_address" class="searchs form-control " />
								</td>
								<td>
									<input type="text" name="registrationstation_phone" id="registrationstation_phone" class="searchs form-control " />
								</td>
								</td><td>
								</td><td>
							</tr>
...
<div class="loading" style="display: none;">
	...
	<div class="blockUI blockMsg blockElement" style="width: 30%;position: absolute;top: 0%;left:35%;text-align: center; z-index: 9999000;">
		<img src="<?=url_tmpl()?>img/ajax_loader.gif" style="position: absolute; z-index: 9999000;"/>
	</div>
</div> 
<!--S Modal -->
...
<script>
	...
	var cpage = 0;
	var search;
	var ttkdt = "<?=getLanguage('ten-tram-khong-duoc-trong');?>";
	$(function(){
		init();
		refresh();
	...
	function searchFunction(){
		$("#registrationstation_name,#registrationstation_code,#registrationstation_phone,#registrationstation_address").keyup(function() {
			searchList();	
		});
	}
	...
	function save(){
		...
		if(obj.registrationstation_name == ''){
			warning(ttkdt); return false;	
		}
		var token = $('#token').val();
		$('.loading').show();
		$.ajax({
    ...
    function funcList(obj){
		$('.edit').each(function(e){
			$(this).click(function(){ 
				 var registrationstation_name = $('.registrationstation_name').eq(e).html().trim();
				 var id = $(this).attr('id');
				 $('#id').val(id);	
				 $('#registrationstation_name').val(registrationstation_name);
			});
		});	
