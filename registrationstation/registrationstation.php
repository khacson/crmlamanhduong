 ...
 * @copyright 2018
 */
class Registrationstation extends CI_Controller {
    private $route;
	private $login;
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('tram-dang-kiem'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['crmd_registrationstation']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['registrationstation_name'];
		$this->base_model->addAcction(getLanguage('tram-dang-kiem'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['registrationstation_name'];
		$this->base_model->addAcction(getLanguage('tram-dang-kiem'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$this->model->table($tb['crmd_registrationstation'])
					->where('id',$id)
					->where('id <> 1')
					->update($array);	
		
		$queryDelete = $this->model->table($tb['crmd_registrationstation'])
					->select('group_concat(registrationstation_name) as deletes')
					->where("id in ($id)")
					->find();
