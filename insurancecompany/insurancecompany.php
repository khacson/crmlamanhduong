...
class Insurancecompany extends CI_Controller {
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('hang-bao-hiem'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['crmd_insurance']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['insurance_name'];
		$this->base_model->addAcction(getLanguage('hang-bao-hiem'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['insurance_name'];
		$this->base_model->addAcction(getLanguage('hang-bao-hiem'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$this->model->table($tb['crmd_insurance'])
					->where('id',$id)
					->where('id <> 1')
					->update($array);	
		
		$queryDelete = $this->model->table($tb['crmd_insurance'])
					->select('group_concat(insurance_name) as deletes')
					->where("id in ($id)")
					->find();
