...
class Manufacturer extends CI_Controller {
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('hang-xe'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['crmd_manufacturer']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['manufacturer_name'];
		$this->base_model->addAcction(getLanguage('hang-xe'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['manufacturer_name'];
		$this->base_model->addAcction(getLanguage('hang-xe'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$this->model->table($tb['crmd_manufacturer'])
					->where('id',$id)
					->where('id <> 1')
					->update($array);	
		
		$queryDelete = $this->model->table($tb['crmd_manufacturer'])
					->select('group_concat(crmd_manufacturer) as deletes')
					->where("id in ($id)")
					->find();
