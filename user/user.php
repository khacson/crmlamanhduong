...
class User extends CI_Controller {
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns('crmd_users');
		} 
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;
	...
	function getDetail(){
		...
		if (!empty($id)) {
			$user = $this->model->table('crmd_users')->where('id', $id)->find();
	...
	function edit() {
		...
		$acction_before = $this->model->base_model->findID('crmd_users',$id);
	...
	function deletes() {
		...
		$array['isdelete'] = 1;
		$this->model->table('crmd_users')->save($id, $array);	
