...
class Priority extends CI_Controller {
	...
	function _view(){ 
		...
		$this->site->write('title',getLanguage('do-uu-tien'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['crmd_priority']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['priority_name'];
		$this->base_model->addAcction(getLanguage('do-uu-tien'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['priority_name'];
		$this->base_model->addAcction(getLanguage('do-uu-tien'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$queryDelete = $this->model->table($tb['crmd_priority'])
					->select('group_concat(priority_name) as priorityname')
					->where("id in ($id)")
					->find();
		 $this->model->table($tb['crmd_priority'])
					->where("id in ($id)")
					->update($array);
		$description = getLanguage('xoa').": ".$queryDelete->priorityname;
		$this->base_model->addAcction(getLanguage('do-uu-tien'),$this->uri->segment(2),'','',$description);
		$result['status'] = 1;	
		$result['csrfHash'] = $token;
		echo json_encode($result);
