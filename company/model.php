 ...
 */
 class CompanyModel extends CI_Model{
	function __construct(){
		parent::__construct('crmd_company');
	...
	function getList($search,$page,$rows){
		...
		$sql = "SELECT c.*, p.province_name, d.distric_name
				FROM crmd_company AS c
				LEFT JOIN crmd_province p on p.id = c.provinceid and p.isdelete = 0
				LEFT JOIN crmd_district d on d.id = c.districid and d.isdelete = 0
				WHERE c.isdelete = 0 
				$searchs
				";
	...
	function getTotal($search){
		...
		FROM crmd_company AS c
		LEFT JOIN crmd_province p on p.id = c.provinceid
		LEFT JOIN crmd_district d on d.id = c.districid 
		WHERE c.isdelete = 0 
		$searchs	
		";
		$query = $this->model->query($sql)->execute();
	...
	function saves($array){
		$this->db->trans_start();
		$check = $this->model->table('crmd_company')
					  ->select('id')
					  ->where('isdelete',0)
		...
		$data = $this->rands(). gmdate("ymdHis",time() + 7 * 3600).$this->site->friendlyURL($array['phone']).$this->site->friendlyURL($array['company_name']);
		$sql = "
				INSERT INTO `crmd_company` 
	...
	function edits($array,$id){
		 ...
		 $result = $this->model->table('crmd_company')->where('id',$id)->update($array);	
		 return $id;
	 }
	function findID($id){
