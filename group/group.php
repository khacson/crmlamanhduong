...
class Group extends CI_Controller {
	...
	function _view(){
		...
		$data->customers = $this->base_model->getCustomer($login->companyid);
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns('crmd_groups');
		} 
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;
	...
	function deletes() {
		...
		$array['userupdate'] = $login->username;
		$array['isdelete'] = 1;
		$this->model->table('crmd_groups')->where("id in ($id)")->where('id <> 1')->update($array);	
