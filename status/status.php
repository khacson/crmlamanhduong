...
class Status extends CI_Controller {
	...
	function _view(){ 
		...
		$this->site->write('title',getLanguage('trang-thai'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['crmd_status']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['status_name'];
		$this->base_model->addAcction(getLanguage('trang-thai'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['status_name'];
		$this->base_model->addAcction(getLanguage('trang-thai'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$queryDelete = $this->model->table($tb['crmd_status'])
					->select('group_concat(status_name) as priorityname')
					->where("id in ($id)")
					->find();
		 $this->model->table($tb['crmd_status'])
					->where("id in ($id)")
					->update($array);
