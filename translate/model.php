...
class TranslateModel extends CI_Model
{
	...
	function findKeyMenu($key) {
        $query = $this->model->table('crmd_menus')
					  ->select('id')
					  ->where('isdelete',0)
					  ->where('keylang',$key)
					  ->find();
	...
	function getLanguage(){
		...
		$query = $this->model->table('crmd_language')
	...
	function saves($array,$id){
		...
		$check = $this->model->table('crmd_translate')
					  ->select('id')
					  ->where('isdelete',0)
		 ...
		 $result = $this->model->table('crmd_translate')->insert($array);	
		 return $result;
	}
	function edits($array,$id){
		...
		$this->model->table('crmd_translate')
					->where('id',$id)
					->update($array);	
		return $id;
