...
class Debts extends CI_Controller {
	...
	function form(){
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tb['sales_liabilities_buy_buy']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function formPay(){
		...
		$tb = $this->base_model->loadTable();
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['sales_liabilities']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
		...
		$data->customers = $this->model->table($tb['sales_customer'])
	...
	function deletes() {
		...
		$checkExit = $this->model->table($tb['sales_receipts'])
									  ->select('uniqueid')
									  ->where('isdelete',0)
		...
		#end
		$acction_before = $this->model->findID($id);
		$this->model->table($tb['sales_liabilities_buy_buy'])
					->where('id',$id)
					->delete();
