...
class Customer extends CI_Controller {
	...
	function _view(){
		...
		$data->customerTypes = $this->model->customerType();		
		$data->customerGroups = $this->model->customerGroup();
	...
	function form(){
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tb['crmd_customer']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = 'Thêm mới: '.$array['customer_name'];
	...
	function edit() {
		...
		$description = 'Sửa: '.$array['customer_name'];
	...
	function deletes() {
		...
		$array['userupdate'] = $this->login->username;
		#region check edit
		$checkExitCus = $this->model->table($tb['crmd_output'])
									  ->select('customerid')
									  ->where("customerid in ($id)")
									  ->where('isdelete',0)
									  ->find();
		if(!empty($checkExitCus->customerid)){
			$result['status'] = -1;	
			$result['csrfHash'] = $token;
			echo json_encode($result); exit;
		}
		#end
		$this->model->table($tb['crmd_customer'])
					->where("id in ($id)")
					->update($array);	
		$skey = $this->login->skey;
		$queryDelete = $this->model->table($tb['crmd_customer'])
					->select("group_concat(DES_DECRYPT(customer_name,'$skey')) as deletes")
					->where("id in ($id)")
					->find();
	...
	function export(){
		...
		foreach ($datas as $item) { 
			...
			$sheetIndex->setCellValueByColumnAndRow(1,$i,$item->customer_code);
			$sheetIndex->setCellValueByColumnAndRow(2,$i,$item->customer_name);
	...
	function import(){ 
		  ...
		  $this->db->trans_start();
		  $provinces = $this->model->table('crmd_province')
							   ->select('friendlyurl,id')
							   ->where('isdelete',0)
							   ->find_combo('friendlyurl','id');	
		  $str_mst = '';  $str_tknh = ''; $str = ''; $str_code = '';					
		  for ($row = 2; $row <= $highestRow; ++$row){
			   $customer_code = trim($objWorksheet->getCellByColumnAndRow(1, $row)->getValue());
			   if(substr($customer_code,0,1)=='='){
					$customer_code = $objWorksheet->getCellByColumnAndRow(1, $row)->getCalculatedValue();
			   }			   
			   if($customer_code == ''){
				     $str =  '- Khách hàng không được trống.<br>';
			   }
			   $str_code.= ",'".$customer_code."'";
		  ...
		  if(!empty($str_mst)){
			  ...
			  $listCus = $this->model->table($tb['crmd_customer'])
					  ->select('taxcode,id')
		  ...
		  if(!empty($str_tknh)){
			  ...
			  $listBankCode = $this->model->table($tb['crmd_customer'])
					  ->select('bankcode,id')
		  ...
		  //Kiem tra khach hang str_code
		   if(!empty($str_code)){
			  ...
			  $listCode = $this->model->table($tb['crmd_customer'])
					  ->select('customer_code,id')
					  ->where('customer_code in ('.$str_code.')')
					  ->where('isdelete',0)
					  ->find_all();
			  if(count($listCode) > 0){
				  $str.= '- Mã khách hàng đã tồn tại:<br>';
				  foreach($listCode as $item){
					   $str.= $item->customer_code .'<br>';
				  }
			  }
		  }
		  $result = new stdClass();
		  ...
		  for ($row = 2; $row <= $highestRow; ++$row){
			   ...
			   $sql.= ",(
			   '".$customer_code."', 
				DES_ENCRYPT('".$customer_name."','$skey'), 
				...
				'".$usercreate."',
				'0')";
				$description = 'Import: '.$customer_name;
				$this->base_model->addAcction('Khách hàng',$this->uri->segment(2),'','',$description);
		  }
		  $sql = substr($sql,1);
		  $insert = "
			INSERT INTO `".$tb['crmd_customer']."` 
				(
				`customer_code`,
				`customer_name`,
				`phone`, 
